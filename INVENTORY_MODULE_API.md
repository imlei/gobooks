...
| **IN.7** | Schema patch — `migrations/084_shipments_add_journal_entry_id.sql` adds the column Phase I.3 wiring wrote to but migration 076 never created. Historical gap silent in dev (test AutoMigrate added it) but blocked `shipment_required=true` first-post in production. Defense-in-depth: registers Receipt / ReceiptLine / Shipment / ShipmentLine / WaitingForInvoiceItem in `db.AutoMigrate` list so future column additions on physical-truth inventory docs are picked up automatically. Unblocks Phase I + Phase I.6 pilot enablement. | Phase I.6 shipped | shipped (local) |
