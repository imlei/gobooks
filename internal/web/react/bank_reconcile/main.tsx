    ...
    <section className="rounded-lg border border-border bg-surface px-4 py-3 shadow-sm">
      <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="mt-1 inline-flex rounded-md border border-border-input bg-background p-0.5">
            {(["all", "payments", "deposits", "split"] as ViewMode[]).map((mode) => (
              <button
                ...
                className={classNames(
                  "rounded px-3 py-1.5 text-small font-semibold capitalize",
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          <span className="text-small text-text-muted2">{visibleCount} visible</span>
          <button type="button" onClick={markAll} className="rounded-md border border-border-input px-3 py-2 text-small font-semibold text-text-muted3 hover:bg-background hover:text-text">Mark visible</button>
          <button type="button" onClick={unmarkAll} className="rounded-md border border-border-input px-3 py-2 text-small font-semibold text-text-muted3 hover:bg-background hover:text-text">Unmark visible</button>
        </div>
      </div>
    </section>
  );
    ...
    <section className="rounded-lg border border-border bg-surface shadow-sm">
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div>
          ...
          <div className="text-small text-text-muted2">{rows.length} transaction(s)</div>
        </div>
      </div>
      <div className="max-h-[62vh] overflow-auto">
        <table className="min-w-full table-fixed text-left text-[12px]">
          <thead className="sticky top-0 z-10 bg-background text-[10px] uppercase tracking-wider text-text-muted">
            <tr className="border-b border-border">
              ...
              <Header label="Ref No." sortKey="reference" current={sortKey} asc={sortAsc} onSort={setSort} className="w-32" />
              ...
              <Header label="Payment" sortKey="payment" current={sortKey} asc={sortAsc} onSort={setSort} className="w-28 text-right" />
              <Header label="Deposit" sortKey="deposit" current={sortKey} asc={sortAsc} onSort={setSort} className="w-28 text-right" />
            </tr>
          </thead>
            ...
            {rows.length === 0 ? (
              ...
              return (
                <tr
                  ...
                  className={classNames(
                    "cursor-pointer border-b border-border-subtle hover:bg-background",
                    checked && "bg-primary-soft/20",
                    row.is_reversal_pair && "text-text-muted2",
                  )}
                >
                  <td className="px-2 py-1.5" onClick={(event) => event.stopPropagation()}>
                    <input type="checkbox" checked={checked} onChange={() => onToggle(row.id)} className="h-4 w-4 rounded border-border-input text-primary" />
                  </td>
                  <td className="truncate px-2 py-1.5 text-text-muted2">{compactDate(row.date)}</td>
                  {!compact ? <td className="truncate px-2 py-1.5">{row.type}{row.is_reversal_pair ? <span className="ml-1 rounded bg-warning-soft px-1 text-[10px] text-warning-hover">Pair</span> : null}</td> : null}
                  ...
                  <td className="truncate px-2 py-1.5">{row.payee || "-"}</td>
                  {!compact ? <td className="truncate px-2 py-1.5 text-text-muted2" title={row.memo}>{row.memo || "-"}</td> : null}
