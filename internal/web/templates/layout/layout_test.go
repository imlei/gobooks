...
func TestLayout_UsesSmartPickerV8(t *testing.T) {
	...
	if !strings.Contains(html, `/static/smart_picker.js?v=8`) {
		t.Fatalf("expected main layout to reference smart_picker.js?v=8, got %q", html)
	}
...
func TestLayoutAuth_UsesSmartPickerV8(t *testing.T) {
	...
	if !strings.Contains(html, `/static/smart_picker.js?v=8`) {
		t.Fatalf("expected auth layout to reference smart_picker.js?v=8, got %q", html)
	}
	if !strings.Contains(html, `window.gobooksFetch`) {
