...
func renderComponentHTML(component templ.Component) string {
...
func renderJournalEntryPageHTML(vm JournalEntryVM) string {
	...
	write(`</div></form></div><script src="/static/journal_entry_fx.js?v=6"></script>`)
	return b.String()
}
...
func renderJournalEntryListHTML(vm JournalEntryListVM) string {
	...
	b.WriteString(`<div class="grid grid-cols-1 gap-3 md:grid-cols-[minmax(220px,1fr)_minmax(220px,320px)_160px_160px_auto] md:items-end">`)
	...
	b.WriteString(`<div class="min-w-0"><div class="text-small font-semibold uppercase tracking-wider text-text-muted">Account</div>`)
	b.WriteString(renderComponentHTML(ui.SmartPicker(ui.SmartPickerVM{
